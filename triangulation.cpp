...
void inTriangle(vector<vector<int>> &graph, int x, vector<bool> &triangle);
...
int main(){
    ...
    for(int i = 0; i < edges; i ++){
        cin >> n1 >> n2;
        if(graph.size() <= max(n1,n2)){
            graph.resize(max(n1,n2) +1);
        }
...
void inTriangle(vector<vector<int>> &graph, int x, vector<bool> &triangle){
    ...
    for(int i = 0; i < graph[x].size(); i++){//iterate through each connecting node to x
        ...
        if(sharedNode(graph[x], graph[index]) != -1){//if the two nodes are a part of the same triangle
            triangle[sharedNode(graph[x], graph[i])] = true; //update node staus in bool array
            triangle[x] = true;
            triangle[index] = true;
            return; 
        }
